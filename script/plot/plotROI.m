function plotROI(gray_matter,savepath,maxColor)
...
for i = 1:6
    ...
    hp = patch('Faces',gray_matter.triangles,...
        ...
        'CDataMapping','scaled','FaceAlpha',0.1);
    hold on
    hp = patch('Faces',ROI_mesh.triangles,...
        'Vertices',ROI_mesh.nodes,'FaceVertexCData',ROI_mesh.node_data{1,1}.data,...
        'FaceColor','interp','EdgeColor','none',...
...
saveas(gcf,fullfile(savepath,'TI_ROI.tif'));
...
hrot = rotate3d;
set(hrot,'ActionPostCallback',@(~,~)camlight(get(gca,'UserData'),'headlight'));
saveas(gcf,fullfile(savepath,'TI_ROI.fig'));
